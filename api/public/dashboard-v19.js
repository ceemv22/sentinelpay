...
function tzFlag(zone) {
    if (zone === 'UTC' || zone === 'auto') return '🌐';
...
async function fetchProfile(token) {
    try {
        ...
        if (tzDd && !tzDd.dataset.bound) {
            ...
            const items = [{ value: 'auto', label: `auto detect (${tzLabel(detectedZone)})`, flag: tzFlag(detectedZone), search: ('auto detect ' + tzLabel(detectedZone)).toLowerCase() }];
            zones.forEach(z => items.push({ value: z, label: tzLabel(z), flag: tzFlag(z), search: tzLabel(z).toLowerCase() }));
            ...
            const renderList = (filter) => {
                ...
                matched.forEach(i => {
                    ...
                    el.innerHTML = `<span class="tz-dd-flag"></span><span class="tz-dd-name"></span>`;
                    el.querySelector('.tz-dd-flag').textContent = i.flag;
            ...
            if (saveTzBtn && !saveTzBtn.dataset.bound) {
                ...
                saveTzBtn.addEventListener('click', () => {
                    localStorage.setItem('sentinel-timezone', tzHidden.value);
