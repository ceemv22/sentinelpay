...
// 0. ULTIMATE HASH SCRUBBER (Runs even before DOMContentLoaded)
const scrubHash = () => {
    if (window.location.hash) {
        // history.replaceState with the split URL is the only way to remove the '#' entirely
        const cleanURL = window.location.href.split('#')[0];
        window.history.replaceState(null, document.title, cleanURL);
    }
};

// Monitor for hash changes (Supabase might re-add it)
window.addEventListener('hashchange', scrubHash);

document.addEventListener('DOMContentLoaded', async () => {
    console.log('[sentinel-dashboard] v7 loader active');
    ...
    // Keep scrubbing for 3 seconds in case Supabase fights back
    let scrubCount = 0;
